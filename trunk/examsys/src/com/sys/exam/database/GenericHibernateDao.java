...
/**
 * <p>Title: GenericHibernateDao.java</p>
 * <p>Description:  </p>
 *
 ...
 *
 * <PRE>
 * <U>Updated by:</U>   auther, 2009-8-15
...
public class GenericHibernateDao<T extends Serializable, PK extends Serializable>
    ...
    public Pager getPager(String hsql, int currentPage, int pageSize_) {
        //  定义pager对象，用于传到页面
        int totalRows = 0;
...
//        totalRows = (query.list()).size(); //取得总计录数 
        //totalRows = this.getRowCount("TbNodeitem");
        totalRows = getAllRowCount(hsql);
        pager = new Pager(totalRows,pageSize_);
        //  如果当前页号为空，表示为首次查询该页
    ...
    public int getAllRowCount(String hql){
        Iterator iterate = getHibernateTemplate().iterate(hql);
        Iterator<Object> objs= iterate;
         int itatol=0;
         while (objs.hasNext())
        {
             objs.next();
    ...
    public List<T> getNumberList(String hql, int number)
    {
        Session session = this.openSession();
